...
const DrfApiFetch = () => {
    ...
    useEffect(() => {
        axios.get('http://127.0.0.1:8000/api/tasks/', {
            headers: {
                'Authorization': 'Token 45091bbeab2cea881495afc0d9d67258e1b8e36d'
            }
        })
    ...
    const deleteTask = (id) => {
        ...
        .then(res => {setTasks(tasks.filter(task => task.id !== id)); setSelectTask([])
            if (editedTask.id === id) {
    ...
    return (
        <div>
            <ul>
                {
                    ...
                    <button onClick={()=>setEditedTask(task)}><i className="fas fa-pen"></i></button>
                    </li>)
            ...
            Set Id<br/>
            <input type='text' value={id} onChange={evt=>(setId(evt.target.value))}/><br/>
            <button type='button' onClick={()=>getTask()}>GetTask</button>
            {/* <button type='button' onClick={()=>deleteTask()}>DeleteTask</button> */}
